...
public class LineGraphView extends View {
    ...
    public void setCircleRadius(float radius) {
        boolean shouldInvalidate = radius != mCircleRadius;
    ...
    private void drawPoints(Canvas canvas) {
        ...
        for (int index=0; index < keys.size(); index++) {
            ...
            drawPoints(canvas, prevPoint, currentPoint);
        ...
        if (mHighlightIntegral && mDrawablePoints.size() > 1) {
            drawHighlightedIntegral(canvas);
        }
    }
    ...
    public void drawPoints(Canvas canvas, DrawablePoint prevPoint, DrawablePoint currPoint) {
        canvas.drawCircle(currPoint.mPoint.x, currPoint.mPoint.y, dpToPx(mCircleRadius), currPoint.mPaintColor);
        if (mShowLines && prevPoint != null) {
